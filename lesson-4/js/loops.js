...
btn.addEventListener('click', function() {
	...
	for (let i = 0; i < cities.length; i ++) {
		...
		if (searchName.toLowerCase() === cities[i].toLowerCase()) {
			/* STEP 2c: If the city is found, output a message to the result paragraph and exit the loop with 'break' */
			result.textContent = `Found ${cities[i]}!`;
			break;
		} else {
			/* STEP 2d: If the city is not found, output a message to the result paragraph */
